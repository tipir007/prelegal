...
/** Fallback used when a required field has not been filled in yet. */
export const PLACEHOLDER = "————"; // "————"

export function fill(value: string, placeholder: string = PLACEHOLDER): string {
  const v = value.trim();
...
function years(value: string): string {
  ...
  return `${fill(value, "___")} ${unit}`;
}

/** Human-readable text for the "MNDA Term" cover-page field. */
export function mndaTermText(d: NdaData): string {
  return d.mndaTerm === "expires"
    ? `Expires ${years(d.mndaTermYears)} from the Effective Date.`
    : "Continues until terminated in accordance with the terms of the MNDA.";
}

/** Human-readable text for the "Term of Confidentiality" cover-page field. */
export function confidentialityText(d: NdaData): string {
  return d.confidentiality === "years"
    ? `${years(
        d.confidentialityYears,
...
/**
 * The Cover Page fields in display order. Single source of truth shared by the
 * on-screen preview (NdaDocument) and the Markdown export (toMarkdown) so the
 * two renderings never drift.
 */
export function coverPageFields(d: NdaData): CoverPageField[] {
  return [
    { label: "Purpose", value: fill(d.purpose) },
    { label: "Effective Date", value: formatEffectiveDate(d.effectiveDate) },
    { label: "MNDA Term", value: mndaTermText(d) },
    { label: "Term of Confidentiality", value: confidentialityText(d) },
    { label: "Governing Law", value: fill(d.governingLaw) },
    { label: "Jurisdiction", value: fill(d.jurisdiction) },
    { label: "MNDA Modifications", value: d.modifications.trim() || "None." },
  ];
}
...
/**
 * The Common Paper Mutual NDA Standard Terms (Version 1.0), free to use under
 * CC BY 4.0. Section 9 is interpolated with the chosen governing law and
 * jurisdiction; the remaining sections are verbatim.
 */
export function standardTerms(d: NdaData): StandardTermSection[] {
  const governingLaw = fill(d.governingLaw, "[Governing Law]");
  const jurisdiction = fill(d.jurisdiction, "[Jurisdiction]");
